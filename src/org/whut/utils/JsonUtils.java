...
//		}
	...
	//2014/5/18新增加接口数据处理：省市区搜索结果数据
		...
		public static List<Device> getEquipInfoDataForDeviceId(String message) throws Exception{
			...
			for(int i=0;i<jsonArray.length();i++){
				JSONObject jsonItem = jsonArray.getJSONObject(i);
				Device device  =  new Device(jsonItem.getString("unitAddress"),jsonItem.getString("equipmentVariety"),Integer.parseInt(jsonItem.getString("riskValue")),jsonItem.getString("userPoint"));
				data.add(device);			
			}	
			return data;
		}
